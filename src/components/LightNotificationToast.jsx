...
export default function LightNotificationToast({ notifications, onDismissed }) {
    ...
    const dismiss = async (recipientId) => {
        ...
        try {
            ...
            setDismissing(null);
            // Tell the bell badge (in Layout.jsx) to refresh right now instead
            // of waiting for its own next 30s poll — otherwise the count
            // looks stuck for up to half a minute after tapping OK.
    ...
    return (
        <>
            {/* Soft visual dim — draws focus to the reminder without blocking
                anything underneath (pointer-events: none, purely visual). */}
            <div className="fixed inset-0 z-[9399] pointer-events-none"
                 style={{ background: "rgba(0,0,0,0.5)", animation: "toastDimIn .25s ease" }} />
            <style>{"@keyframes toastDimIn{from{opacity:0}to{opacity:1}}"}</style>
            ...
            <div className="fixed z-[9400] flex flex-col gap-2 pointer-events-none"
                ...
                {visible.map(n => (
                    <div key={n.recipientId}
                         className="pointer-events-auto bg-slate-800 border-2 border-amber-400/70
                                rounded-2xl overflow-hidden"
                         style={{
                             animation: "toastSlideIn .25s ease",
                             boxShadow: "0 0 0 1px rgba(251,191,36,0.15), 0 8px 32px rgba(0,0,0,0.6), 0 0 24px rgba(251,191,36,0.15)",
                         }}>
